//! Validating a URL before it is handed to the OS.
//!
//! Nothing here spawns a process — that stays with the desktop opener, which
//! knows how to hand a validated string to the platform. This module is the
//! load-bearing check: the scheme allowlist that stops `file:///C:/Users/.../
//! id_rsa` and `javascript:`, and the character rules that stop the argument
//! being anything but one argument.
...
pub(crate) fn strip_http_scheme(raw: &str) -> Option<&str> {
